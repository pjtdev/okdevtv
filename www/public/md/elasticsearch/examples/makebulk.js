...
for (idx in dl) {
    ...
    fs.appendFile('myfile.txt', row, (err, fd) => {
        if (err) {
            if (err.code === "EEXIST") {
...
function getJson(line) {
  ...
  'cs_content'];
    ...
    if (t.cs_content) {
        ...
        t.cat5 = (carray[1] && carray[1].indexOf('<') > -1) ?
            carray[1].split('<')[1] : carray[1];
    }
    var d = t.date;
    var d2 = (t.date) ?
        d.substring(0, 4) + '-' + d.substring(4, 6) + '-' + d.substring(6, 8) : '2017-01-01';
